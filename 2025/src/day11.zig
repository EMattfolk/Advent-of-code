...
fn all_paths(paths: *std.hash_map.AutoHashMap([3]u8, [][3]u8), goal: [3]u8, node: [3]u8) i64 {
    if (std.mem.eql(u8, &node, &goal)) {
        return 1;
    }

    var res: i64 = 0;
    if (paths.get(node)) |slice| {
        for (slice) |child| {
            res += all_paths(paths, goal, child);
        }
    }
    return res;
}

fn can_reach(paths: *std.hash_map.AutoHashMap([3]u8, [][3]u8), visited: *std.hash_map.AutoHashMap([3]u8, void), goal: [3]u8, node: [3]u8) !bool {
    if (std.mem.eql(u8, &node, &goal)) {
        return true;
    }
    ...
    if (paths.get(node)) |slice| {
        for (slice) |child| {
            res = res or try can_reach(paths, visited, goal, child);
        }
    }
    return res;
}
...
fn solve_impl(input: []const u8) !struct { []u8, []u8 } {
    ...
    ans1 = all_paths(&paths, @constCast("out").*, @constCast("you").*);
    ...
    var visited = std.AutoHashMap([3]u8, void).init(lib.a);
    var toRemove: std.ArrayList([3]u8) = .empty;
    var it2 = paths.keyIterator();
    while (it2.next()) |node| {
        if (!try can_reach(&paths, &visited, @constCast("fft").*, node.*)) {
            try toRemove.append(lib.a, node.*);
        }
        visited.clearRetainingCapacity();
    }

    var pathsToFft: std.hash_map.AutoHashMap([3]u8, [][3]u8) = try paths.clone();
    for (toRemove.items) |r| {
        _ = pathsToFft.remove(r);
    }

    // std.debug.print("{}\n", .{toRemove.items.len});

    ans2 = all_paths(&pathsToFft, @constCast("fft").*, @constCast("svr").*);

    toRemove.shrinkRetainingCapacity(0);

    it2 = paths.keyIterator();
    while (it2.next()) |node| {
        if (!try can_reach(&paths, &visited, @constCast("dac").*, node.*)) {
            try toRemove.append(lib.a, node.*);
        }
        visited.clearRetainingCapacity();
    }

    var pathsToDac: std.hash_map.AutoHashMap([3]u8, [][3]u8) = try paths.clone();
    for (toRemove.items) |r| {
        _ = pathsToDac.remove(r);
    }

    ans2 *= all_paths(&pathsToDac, @constCast("dac").*, @constCast("fft").*);
    ans2 *= all_paths(&paths, @constCast("out").*, @constCast("dac").*);
